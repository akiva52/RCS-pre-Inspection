...
export default function Setup() {
  ...
  function cleanAddress(item) {
    ...
    const stateZip = [a.state_code || a.state, a.postcode].filter(Boolean).join(' ')
  ...
  async function handleAddressChange(val) {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        const cleaned = data.map(d => ({ display: cleanAddress(d) }))
        ...
        <div className="setup-screen">
          ...
          <div className="form-group" style={{position:'relative'}}>
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div style={{position:'absolute',top:'100%',left:0,right:0,background:'#fff',border:'1px solid var(--border)',borderRadius:'10px',zIndex:50,boxShadow:'0 4px 16px rgba(0,0,0,0.1)',maxHeight:'200px',overflowY:'auto'}}>
                {suggestions.map((s, i) => (
                  <div key={i} onMouseDown={() => selectSuggestion(s.display)}
                    style={{padding:'10px 14px',fontSize:'12px',color:'var(--text)',borderBottom:i<suggestions.length-1?'1px solid var(--border)':'none',cursor:'pointer',lineHeight:'1.4'}}>
                    {s.display}
                  </div>
                ))}
              </div>
          ...
          <div style={{marginBottom:'8px',fontSize:'12px',color:'var(--muted)'}}>Add each wing or section of the building and how many floors it has.</div>
          {wings.map((wing, idx) => (
